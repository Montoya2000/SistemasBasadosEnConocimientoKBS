...
(defrule text-cust
    ...
    =>
    (assert (text-customer (name ?name) (phone ?phone) (message "tienes 25% desc prox compra")))
    (printout t ?name " " ?phone " tienes 25% desc prox compra" crlf))

(defrule call-cust
    (customer (customer-id ?cid) (name ?name) (phone ?phone))
    (not (order (order-number ?order) (customer-id ?cid)))
    =>
    (assert (call-customer (name ?name) (phone ?phone) (message "tienes 25% desc prox compra")))
    (printout t ?name " " ?phone " tienes 25% desc prox compra" crlf))
